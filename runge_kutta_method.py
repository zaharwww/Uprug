...
class RungeKutta:
    ...
    def runge_kutta_fast_step(self, circle_object, step, end_time, start_time):
        ...
        for g in range(num_points):
            while start_time < end_time:
                k_11 = - np.exp(start_time) * circle_object.vector_of_points_inside_circle_x[g]
                t_11 = start_time + (2/3) * step
                x_11 = circle_object.vector_of_points_inside_circle_x[g] + ((2/3) * step * k_11)
                k_12 = - np.exp(t_11) * x_11
                x_12 = circle_object.vector_of_points_inside_circle_x[g] + step * ((-1/3) * k_11 + 1 * k_12)
                k_13 = - np.exp(t_11) * x_12
                x_11_final = circle_object.vector_of_points_inside_circle_x[g] + step * ((1/4) * k_11 + (2/4) * k_12 + (1/4) * k_13)
                list_of_coordinate_x_for_several_points.append(float(x_11_final))

                k_21 = np.exp(start_time) * circle_object.vector_of_points_inside_circle_y[g]
                t_21 = start_time + (2/3) * step
                x_21 = circle_object.vector_of_points_inside_circle_y[g] + ((2/3) * step * k_21)
                k_22 = np.exp(t_21) * x_21
                x_22 = circle_object.vector_of_points_inside_circle_y[g] + step * ((-1/3) * k_21 + 1 * k_22)
                k_23 = np.exp(t_21) * x_22
                x_21_final = circle_object.vector_of_points_inside_circle_y[g] + step * ((1/4) * k_21 + (2/4) * k_22 + (1/4) * k_23)
